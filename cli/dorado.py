...
def main():
    ...
    p = sub.add_parser("inspect-hook", help="Inspect (MIT) signed-receipt SCORER hook (move 59) — attach a signed a2a score receipt riding a Score result, or stranger-verify one. Provenance rides, never alters, the measurement (measurement, never certification)")
    p.add_argument("--in", dest="inp", default=None, help="An Inspect Score JSON to sign")
    p.add_argument("--verify", action="store_true", help="Stranger-verify the signed receipt riding `--in` (no signing key)")
    p.add_argument("--key-file", default=None, help="Pod Ed25519 private key (repo never embeds it)")
    p.add_argument("--kid", default=None, help="kid to stamp (default did:web:csoai.org#test-identity)")
